...
CreareGame = () => {
  ...
  marime = 25 / nrLinii;
  timp = 0;
  TimpInterval = setInterval(function () {
    timp++;
    Timp.innerHTML = `Timp :${timp}s`;
  }, 1000);
  matrice2 = new Array();
  for (i = 0; i < nrColoane; i++) matrice2[i] = new Array();

  matrice = new Array();
  for (i = 0; i < nrColoane; i++) matrice[i] = new Array();
  ...
  for (i = 0; i < nrLinii; i++) {
    for (j = 0; j < nrColoane; j++) {
      if (matrice[i][j] != `<img src="Bomba.png">`) {
        nrBombe = 0;
        if (i + 1 < nrLinii) {
          if (matrice[i + 1][j] == `<img src="Bomba.png">`) nrBombe++;
        }
        if (i - 1 > -1) {
          if (matrice[i - 1][j] == `<img src="Bomba.png">`) nrBombe++;
        }
        if (j + 1 < nrColoane) {
          if (matrice[i][j + 1] == `<img src="Bomba.png">`) nrBombe++;
        }
        if (j - 1 > -1) {
          if (matrice[i][j - 1] == `<img src="Bomba.png">`) nrBombe++;
        }
        if (i + 1 < nrLinii && j + 1 < nrColoane) {
          if (matrice[i + 1][j + 1] == `<img src="Bomba.png">`) nrBombe++;
        }
        if (i - 1 > -1 && j - 1 > -1) {
          if (matrice[i - 1][j - 1] == `<img src="Bomba.png">`) nrBombe++;
        }
...
CuloareCifra = (i, j) => {
  if (matrice[i][j] == 1) matrice2[i][j].style.color = "var(--color1)";
  if (matrice[i][j] == 2) matrice2[i][j].style.color = "var(--color2)";
  if (matrice[i][j] == 3) matrice2[i][j].style.color = "var(--color3)";
  if (matrice[i][j] == 4) matrice2[i][j].style.color = "var(--color4)";
  if (matrice[i][j] == 5) matrice2[i][j].style.color = "var(--color5)";
  if (matrice[i][j] == 6) matrice2[i][j].style.color = "var(--color6)";
};
...
AfiseareVeciniNeapasati = () => {
  if (matrice2[i][j].presed) {
    if (i + 1 < nrLinii) {
      if (
        matrice2[i + 1][j].innerHTML != `<img src="Steag2.png">` &&
        !matrice2[i + 1][j].presed
      ) {
        matrice2[i + 1][j].style.backgroundColor = "rgb(108, 159, 252)";
        setTimeout(function () {
          matrice2[i + 1][j].style.backgroundColor = "rgb(49, 115, 238)";
        }, 100);
      }
    }
    if (i - 1 > -1) {
    ...
    if (j + 1 < nrColoane) {
      if (
        matrice2[i][j + 1].innerHTML != `<img src="Steag2.png">` &&
        !matrice2[i][j + 1].presed
      ) {
        matrice2[i][j + 1].style.backgroundColor = "rgb(108, 159, 252)";
        setTimeout(function () {
          matrice2[i][j + 1].style.backgroundColor = "rgb(49, 115, 238)";
        }, 100);
      }
    }
    if (j - 1 > -1) {
    ...
    if (i + 1 < nrLinii && j + 1 < nrColoane) {
      if (
        matrice2[i + 1][j + 1].innerHTML != `<img src="Steag2.png">` &&
        !matrice2[i + 1][j + 1].presed
      ) {
        matrice2[i + 1][j + 1].style.backgroundColor = "rgb(108, 159, 252)";
        setTimeout(function () {
          matrice2[i + 1][j + 1].style.backgroundColor = "rgb(49, 115, 238)";
        }, 100);
      }
    }
    if (i - 1 > -1 && j - 1 > -1) {
